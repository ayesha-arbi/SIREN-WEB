...
// ─── StatCard ─────────────────────────────────────────────────────────────────
interface StatProps { label: string; value: string | number; change?: string; color: 'blue'|'red'|'green'|'orange'; icon?: string }
...
export function Badge({ variant, children }: { variant: BadgeVariant; children: ReactNode }) {
  const cls: Record<BadgeVariant, string> = {
    ...
    blue: 'badge-blue', active: 'badge-active',
    yellow: 'badge-yellow',
  }
...
export function ConfidenceBar({ score, severity }: { score: number; severity: Crisis['severity'] }) {
  const colors: Record<Crisis['severity'], string> = {
    high: 'var(--red-hot)', medium: 'var(--orange)', low: 'var(--green-ok)'
  }
  return (
...
// ─── Map canvas (CSS-based) ───────────────────────────────────────────────────
interface MapProps { height?: number; children?: ReactNode }
export function MapCanvas({ height = 380, children }: MapProps) {
  return (
    <div className="map-container" style={{ height }}>
      <div className="map-bg-gradient" />
      <div className="map-road road-h1" /><div className="map-road road-h2" />
      <div className="map-road road-h3" /><div className="map-road road-v1" />
      <div className="map-road road-v2" /><div className="map-road road-v3" />
      {children}
    </div>
  )
}
